import { TestData } from "../../interfaces/test.interface";

const anime: TestData = {
    title: "Anime",
    questions: [
      {
        timeSpent: 0,
        question: "Kto jest twórcą słynnego anime 'Naruto'?",
        correctAnswer: 2,
        answers: [
          { content: "Hayao Miyazaki", id: 1 },
          { content: "Masashi Kishimoto", id: 2 },
          { content: "Mamoru Hosoda", id: 3 },
          { content: "Isao Takahata", id: 4 },
        ],
      },
      {
        timeSpent: 0,
        question:
          "Co oznacza termin 'manga' w kontekście japońskiej kultury?",
          correctAnswer: 2,
        answers: [
          { content: "Anime filmowe", id: 1 },
          { content: "Komiks japoński", id: 2 },
          {
            content: "Tradycyjna herbata",
            id: 3,
          },
          { content: "Styl sztuk walki", id: 4 },
        ],
      },
      {
        timeSpent: 0,
        question: "Które z poniższych anime należy do gatunku 'mecha'?",
        correctAnswer: 2,
        answers: [
          { content: "One Piece", id: 1 },
          { content: "Gundam Wing", id: 2 },
          { content: "Death Note", id: 3 },
          { content: "My Hero Academia", id: 4 },
        ],
      },
      {
        timeSpent: 0,
        question:
          "Które anime opowiada historię o chłopcu, który staje się mistrzem Pokemonów?",
          correctAnswer: 4,
        answers: [
          { content: "Dragon Ball", id: 1 },
          { content: "One Punch Man", id: 2 },
          { content: "Naruto", id: 3 },
          { content: "Pokémon", id: 4 },
        ],
      },
      {
        timeSpent: 0,
        question: "Co to jest 'cosplay' w kontekście kultury anime?",
        correctAnswer: 3,
        answers: [
          {
            content: "Typ japońskiego jedzenia",
            id: 1,
          },
          { content: "Sztuka origami", id: 2 },
          {
            content: "Ubieranie się w stroje postaci z anime",
            id: 3,
          },
          {
            content: "Japońska sztuka walki",
            id: 4,
          },
        ],
      },
    ],
  }
  export default anime;