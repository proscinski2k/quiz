import { type QuizData } from '../../interfaces/quiz.interface'

const religion: QuizData = {
    title: 'Religia',
    questions: [
        {
            timeSpent: 0,
            question: 'Kto jest uznawany za założyciela chrześcijaństwa?',
            correctAnswer: 1,
            answers: [
                { content: 'Jezus Chrystus', id: 1 },
                { content: 'Mahawira', id: 2 },
                { content: 'Budda Siddhartha Gautama', id: 3 },
                { content: 'Mohamed', id: 4 }
            ]
        },
        {
            timeSpent: 0,
            question:
                'Które z poniższych świąt jest obchodzone przez katolików?',
            correctAnswer: 1,
            answers: [
                { content: 'Boże Narodzenie', id: 1 },
                { content: 'Diwali', id: 2 },
                { content: 'Ramadan', id: 3 },
                { content: 'Hanuka', id: 4 }
            ]
        },
        {
            timeSpent: 0,
            question: 'Co to jest Biblia?',
            correctAnswer: 2,
            answers: [
                { content: 'Święta księga islamu', id: 1 },
                { content: 'Biblia katolicka', id: 2 },
                { content: 'Księga Mormona', id: 3 },
                { content: 'Stary Testament', id: 4 }
            ]
        },
        {
            timeSpent: 0,
            question:
                'Które z poniższych miejsc uważane jest za święte przez katolików?',
            correctAnswer: 3,
            answers: [
                { content: 'Jerozolima', id: 1 },
                { content: 'Mekka', id: 2 },
                { content: 'Watykan', id: 3 },
                { content: 'Rzym', id: 4 }
            ]
        },
        {
            timeSpent: 0,
            question:
                'Kto jest uznawany za głównego proroka w chrześcijaństwie?',
            correctAnswer: 4,
            answers: [
                { content: 'Budda', id: 1 },
                { content: 'Jezus Chrystus', id: 2 },
                { content: 'Mohamed', id: 3 },
                { content: 'Jan Chrzciciel', id: 4 }
            ]
        }
    ]
}

export default religion
